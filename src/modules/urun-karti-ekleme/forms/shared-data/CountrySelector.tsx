...
function CountrySelector({ value, onChange, placeholder = "Menşei seçin veya yazın...", className = "", required = false }: CountrySelectorProps) {
  ...
  const filteredCountries = searchTerm.trim() 
    ...
    : [...getPopularCountries(), ...countries.filter(c => !c.popular && c.id !== 'diger')];
    ...
    <div className={`relative ${className}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-60 overflow-y-auto">
          ...
          {(searchTerm ? filteredCountries : countries.filter(c => !c.popular && c.id !== 'diger'))
            .map((country, index) => {
                ...
                >
                  ...
                  <span>
                    {searchTerm ? (
                      ...
                      country.name.split(new RegExp(`(${searchTerm})`, 'gi')).map((part, i) => 
                  ...
                  {country.region && (
                    <span className="text-xs text-gray-400 ml-2">({country.region})</span>
                  )}
                </button>
              );
            })}
          ...
          {!searchTerm && (
            <>
              ...
              <button
                type="button"
                onClick={() => handleSelect('Diğer')}
                className={`w-full text-left px-3 py-2 hover:bg-blue-50 focus:bg-blue-50 focus:outline-none ${
                  highlightedIndex === filteredCountries.length - 1 ? 'bg-blue-50' : ''
                }`}
              >

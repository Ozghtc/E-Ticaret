...
export function getStepTitle(step: number): string {
  switch (step) {
    ...
    case 4:
      return t("common.önizleme_kaydet");
    default:
      return t("common.bilinmeyen_adım");
  }
}
...
export function hasShadesField(categoryId: string): boolean {
  const shadesCategories = ['fondoten', 'ruj', 'rimmel', 'far', 'kapatici', 'pudra', 'allik', 'kas-kalemi', 'eyeliner', 'highlighter', 'oje'];
  return shadesCategories.includes(categoryId);
}
export function hasVolumeField(categoryId: string): boolean {
  const volumeCategories = ['nemlendirici', 'temizleyici', 'maske', 'serum', 'tonik', 'gunes-kremi', 'peeling', 'goz-kremi', 'fondoten', 'kapatici', 'pudra', 'sampuan', 'sac-kremi', 'sac-maskesi', 'sac-yagi', 'sac-spreyi', 'sac-serumu', 'kuru-sampuan', 'dus-jeli', 'vucut-losyonu', 'deodorant', 'vucut-yagi', 'el-kremi', 'vucut-spreyi', 'oje', 'base-coat', 'top-coat', 'tirnak-bakimi', 'asetik-dis', 'kadin-parfum', 'erkek-parfum', 'unisex-parfum', 'body-mist'];
...
export function validateImageFile(file: File): {
  ...
  if (!allowedTypes.includes(file.type)) {
    return {
      valid: false,
      error: t("common.sadece_jpg_png_veya_webp_formatında_dosya_yükleyebilirsiniz")
    };
  }
  if (file.size > maxSize) {
    return {
      valid: false,
      error: t("common.dosya_boyutu_5mb_dan_küçük_olmalıdır")
    };
  }
  return {
    valid: true

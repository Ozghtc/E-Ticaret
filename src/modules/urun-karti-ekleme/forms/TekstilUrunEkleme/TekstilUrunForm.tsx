...
function TekstilUrunForm() {
  ...
  const handleSubmit = () => {
    ...
    showToastNotification(t("common.ürün_başarıyla_eklendi_ürün_yönetimi_sayfasına_yönlendiriliyorsunuz"), 'success');
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case 2:
        return <VaryantFiyatStep formData={formData} updateFormData={updateFormData} variantHook={variantHook} />;
      case 3:
        return <UrunGorselleriStep formData={formData} variants={variantHook.variants} updateFormData={updateFormData} />;
      case 4:
        return <OnizlemeKaydetStep formData={formData} variants={variantHook.variants} updateFormData={updateFormData} />;
      default:
        return null;
    }
  };
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="bg-white rounded-xl shadow-lg p-8">
          ...
          <div className="flex justify-between mt-8 pt-6 border-t border-gray-200">
            ...
            {currentStep < totalSteps ? <button onClick={handleNext} disabled={!isStepValid(currentStep, formData)} className={`px-6 py-3 rounded-lg font-medium flex items-center space-x-2 ${isStepValid(currentStep, formData) ? 'bg-pink-600 text-white hover:bg-pink-700' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}>

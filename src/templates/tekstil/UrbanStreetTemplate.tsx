...
export default function UrbanStreetTemplate({
  products,
  category
}: UrbanStreetTemplateProps) {
  const {
    t
  } = useTranslation();
  ...
  return <div className="min-h-screen bg-black">
      ...
      <section className="py-16 bg-gradient-to-r from-red-900 to-black">
        <div className="max-w-6xl mx-auto px-4 text-center">
          ...
          <p className="text-xl text-red-200 mb-8">{t("common.sokakların_ritmi_gençliğin_enerjisi")}</p>
          <button className="bg-red-600 text-white px-8 py-4 font-bold text-lg hover:bg-red-700 transition-colors transform hover:scale-105">{t("common.koleksiyonu_gör")}</button>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map(product => <ProductCard key={product.id} product={product} category={category} theme="urban-street" />)}
        </div>
      </section>
    </div>;
}
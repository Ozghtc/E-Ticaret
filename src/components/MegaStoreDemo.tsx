...
import { ArrowLeft, Search, ShoppingCart, User, Heart, Star, Filter, Grid3X3, List, ChevronDown, Truck, Shield, RotateCcw, Phone } from 'lucide-react';
function MegaStoreDemo() {
  ...
  const products = [{
    ...
    id: 5,
    name: t("common.sony_wh_1000xm5_kulaklık"),
    price: 8999,
    originalPrice: 10999,
    ...
    id: 6,
    name: t("common.dyson_v15_detect_süpürge"),
    price: 15999,
    originalPrice: 18999,
      ...
      <header className="bg-white shadow-sm sticky top-0 z-50">
        ...
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            {/* Back Button & Logo */}
            <div className="flex items-center space-x-4">
              <button onClick={() => navigate('/admin/tema-sistemi')} className="flex items-center text-gray-600 hover:text-red-600 transition-colors">
        ...
        <div className="flex gap-8">
              ...
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  {categories.map(category => <label key={category.id} className="flex items-center">
                      <input type="radio" name="category" value={category.id} checked={selectedCategory === category.id} onChange={e => setSelectedCategory(e.target.value)} className="text-red-600 focus:ring-red-500" />
          ...
          <main className="flex-1">
            ...
            <div className="bg-white rounded-lg shadow-sm p-4 mb-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  <span className="text-gray-600">{t("common.1_247_ürün_bulundu")}</span>
                ...
                <div className="flex items-center space-x-2">
                  <button onClick={() => setViewMode('grid')} className={`p-2 rounded ${viewMode === 'grid' ? 'bg-red-100 text-red-600' : 'text-gray-400'}`}>
                    <Grid3X3 size={16} />
                  </button>
                  <button onClick={() => setViewMode('list')} className={`p-2 rounded ${viewMode === 'list' ? 'bg-red-100 text-red-600' : 'text-gray-400'}`}>
                    <List size={16} />
                  </button>
                </div>
              </div>
            ...
            <div className={`grid gap-6 ${viewMode === 'grid' ? 'grid-cols-1 md:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'}`}>
              {products.map(product => <div key={product.id} className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden group">
                  <div className="relative">
                    ...
                    {product.discount > 0 && <div className="absolute top-2 left-2 bg-red-500 text-white px-2 py-1 rounded text-sm font-medium">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center space-x-2 text-xs text-green-600">
                        {product.freeShipping && <div className="flex items-center">
                            <Truck size={12} className="mr-1" />
                            <span>Ücretsiz Kargo</span>
                          </div>}
                        {product.fastDelivery && <div className="flex items-center">
                            <span>{t("common.hızlı_teslimat")}</span>
                          </div>}
                      </div>
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div className="text-center">
              ...
              <p className="text-gray-600 text-sm">{t("common.256_bit_ssl_şifreleme")}</p>
            </div>
            <div className="text-center">
              ...
              <p className="text-gray-600 text-sm">{t("common.15_gün_içinde_ücretsiz")}</p>
            </div>
            <div className="text-center">
              ...
              <p className="text-gray-600 text-sm">{t("common.canlı_destek_hattı")}</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            <div>
              ...
              <p className="text-gray-400 mb-4">{t("common.türkiye_nin_en_büyük_online_alışveriş_platformu")}</p>
            ...
            <div>
              ...
              <ul className="space-y-2 text-gray-400">
                <li><a href="#" className="hover:text-white">{t("common.hakkımızda")}</a></li>
                <li><a href="#" className="hover:text-white">Kariyer</a></li>
                <li><a href="#" className="hover:text-white">{t("common.basın")}</a></li>
                <li><a href="#" className="hover:text-white">{t("common.i_letişim")}</a></li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold mb-4">{t("common.yardım")}</h4>
              <ul className="space-y-2 text-gray-400">
                <li><a href="#" className="hover:text-white">{t("common.sıkça_sorulanlar")}</a></li>
                <li><a href="#" className="hover:text-white">Kargo & Teslimat</a></li>
                <li><a href="#" className="hover:text-white">{t("common.i_ade_değişim")}</a></li>
                <li><a href="#" className="hover:text-white">{t("common.güvenlik")}</a></li>
              </ul>
            </div>
            <div>
              <h4 className="font-semibold mb-4">{t("common.hesabım")}</h4>
              <ul className="space-y-2 text-gray-400">
                <li><a href="#" className="hover:text-white">{t("common.giriş_yap")}</a></li>
                <li><a href="#" className="hover:text-white">{t("common.siparişlerim")}</a></li>
          ...
          <div className="border-t border-gray-800 mt-8 pt-8 text-center text-gray-400">
            <p>{t("common.2025_megastore_tüm_hakları_saklıdır_bu_demo_altıntassoft_tarafından_oluşturulmuştur")}</p>
          </div>
        </div>
      </footer>
    </div>;
